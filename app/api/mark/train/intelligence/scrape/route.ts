/**
 * POST /api/mark/train/intelligence/scrape
 *
 * Implements the Jens Technique for a given Instagram account.
 * Scrapes the artist's full public post history via Apify, runs pattern
 * analysis, and returns a structured Tier 3 context string for Mark.
 *
 * Input:  { username: string }   — IG handle, with or without @
 * Output: { accountSummary, tier3Context, topPosts, insights, rawPostCount }
 */
...
function buildAccountSummary(posts: AnalyzedPost[], username: string): AccountSummary {
  if (posts.length === 0) {
    return {
      ...
      topFormats: [], captionInsights: [], growthSignal: 'No posts found.',
    };
  }
  ...
  // Growth signal (very rough — based on recency performance)
  const recent = [...posts].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, 10);
  const older = [...posts].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(-10);
  const recentAvgER = recent.reduce((s, p) => s + p.er, 0) / (recent.length || 1);
  const olderAvgER = older.reduce((s, p) => s + p.er, 0) / (older.length || 1);
  const growthSignal = recentAvgER > olderAvgER * 1.1
    ? `Engagement trending UP — recent posts averaging ${recentAvgER.toFixed(2)}% ER vs ${olderAvgER.toFixed(2)}% on older posts`
    : recentAvgER < olderAvgER * 0.9
    ? `Engagement trending DOWN — recent posts averaging ${recentAvgER.toFixed(2)}% ER vs ${olderAvgER.toFixed(2)}% on older posts`
    : `Engagement stable at ~${avgER.toFixed(2)}% ER`;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const raw = await scrapeProfile(username.trim());
    ...
    const summary = buildAccountSummary(analyzed, username.replace(/^@/, ''));
    const tier3Context = buildTier3Context(analyzed, summary, username.replace(/^@/, ''));

    // Top posts to show in UI
    ...
    return NextResponse.json({ accountSummary: summary, tier3Context, topPosts, rawPostCount: raw.length });

  } catch (e: any) {
    console.error('[intelligence/scrape]', e);
